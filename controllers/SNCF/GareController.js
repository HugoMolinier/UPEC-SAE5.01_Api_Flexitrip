const { Gare } = require("../../models/SNCF");
...
exports.getAllGare = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la récupération des gares" });
  }
};
...
exports.getGareById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la récupération de la gare" });
  }
};

// Créer une gare
exports.createGare = async (req, res) => {
  const { name, city, country, iata_code, icao_code, latitude, longitude } =
    req.body;
  try {
    const newgare = await Gare.create({
      name,
      city,
      country,
      iata_code,
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la création de la gare" });
  }
};
...
exports.updateAirport = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la mise à jour de la gare" });
  }
};
...
exports.deleteGare = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur serveur lors de la suppression de la gare" });
  }
};
